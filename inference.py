...
try:
    from client import get_env_client
except ImportError:
    pass
...
def log_end(success: bool, steps: int, score: float, rewards: List[float], cost: float = 0.0, downtime: float = 0.0):
    # Strictly match sample inference script [END] format for evaluator regex parser
    print(f"[END] success={success} steps={steps} score={score:.2f} rewards={rewards}")
    # Additional SLA metrics (not parsed by evaluator)
...
def get_rule_based_action(last_obs: DevOpsObservation) -> str:
    # Prioritize critical severity services first
...
async def main() -> None:
    ...
    # Run all 3 tasks across all 3 agent types
